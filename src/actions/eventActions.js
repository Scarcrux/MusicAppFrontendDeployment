...
const createEvent = (streetName, city, state, zip, headline, description, date, user_id) => async (dispatch, getState) => {
    try {
      ...
      const { data } = await Axios.post("http://127.0.0.1:5000/createevent", { streetName, city, state, zip, headline, description, date, user_id },{
        headers: {
  ...
  const allEvents = (page, searchTerm) => async (dispatch) => {
    try {
      ...
      const { data } = await Axios.get("http://127.0.0.1:5000/events/"+page+"?term="+searchTerm, {},{});
  ...
  const getEvent = (id) => async (dispatch) => {
    try {
      ...
      const { data } = await Axios.get("http://127.0.0.1:5000/event/"+id, {},{});
